...
func (f *Factory) createMQTTTransport() (*TransportSetup, error) {
	...
	publisher := NewMQTTPublisher(mqttClient, f.config.DomainID, f.config.Channels.CtrlChan(), TopicControl)
	connector := NewMQTTConnector(mqttClient, f.config.DomainID, f.config.Channels.CtrlChan(), TopicControl)

	broker := conn.NewBroker(f.service, mqttClient, f.config.Channels.CtrlChan(), f.config.DomainID, f.logger)
	mqttBroker := NewMQTTBroker(broker, mqttClient, f.config.DomainID, f.config.Channels.CtrlChan(), TopicControl)
...
func (f *Factory) createCoAPTransport() (*TransportSetup, error) {
	...
	publisher := NewCoAPPublisher(coapClient, f.config.DomainID, f.config.Channels.CtrlChan(), TopicControl, f.config.CoAP.ContentFormat)
	...
	coapBroker := NewCoAPBroker(broker, coapClient, f.config.DomainID, f.config.Channels.CtrlChan(), TopicControl, f.config.CoAP.ContentFormat)
